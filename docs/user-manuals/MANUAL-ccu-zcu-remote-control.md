...
- **Watchdog: tự khởi động lại app kiosk khi đóng/crash** *(mới — tick sẵn mặc định):* cài một dịch vụ nền (systemd) giám sát phần mềm kiosk. Nếu phần mềm bị đóng hoặc gặp sự cố (crash), dịch vụ sẽ **tự mở lại phần mềm sau khoảng 10 giây**, không để lộ màn hình nền trống. Dịch vụ **không bao giờ tự bỏ cuộc**: kể cả khi phần mềm lỗi liên tục hoặc chưa được cài, nó vẫn kiên trì thử lại mỗi 10 giây (máy kiosk không có người trực — thà thử mãi còn hơn dừng hẳn và bỏ trống màn hình); nhịp 10 giây đủ chậm để không làm đầy nhật ký hệ thống. Khi bật Watchdog, phần mềm được dịch vụ này quản lý — autostart `.desktop` của phần mềm được tự động gỡ bỏ (tránh chạy 2 bản cùng lúc), bất kể ô Autostart ở trên. Bỏ tick để gỡ dịch vụ (phần mềm quay về tự mở bằng autostart `.desktop` nếu còn tick ô ở trên).

> 📱 **Lưu ý cho máy MÀN HÌNH CẢM ỨNG (không có bàn phím):** ô tick **"Ẩn nút Activities"** (Tab "🖥️ Config máy tính") ngoài việc ẩn nút Activities còn cài thêm một tiện ích **được cấu hình để chặn** các **cử chỉ cảm ứng** mở màn hình Activities overview (vuốt nhiều ngón, vuốt từ mép, chạm-giữ, di góc màn hình). Trên GNOME, các cử chỉ này không thể tắt bằng cấu hình thông thường — tiện ích vô hiệu màn hình overview theo mọi cách gọi, trong khi **thao tác chạm 1 ngón để dùng phần mềm vẫn hoạt động bình thường**. Cần **khởi động lại máy** để tiện ích có hiệu lực, và nên **nghiệm thu trực tiếp tại máy** sau khi cài: thử vuốt nhiều ngón / chạm-giữ trên màn hình cảm ứng để xác nhận overview không còn mở được. Nếu là máy có bàn phím, kết hợp thêm ô **"Khoá lối thoát kiosk (dconf lock)"** để chặn các phím tắt.
...
- **Super+1..9** (mở nhanh ứng dụng ghim trong Favorites — có thể gồm Terminal/Files), **Alt+`** (chuyển cửa sổ cùng ứng dụng), **Alt+Space** (menu cửa sổ có nút Close/Minimize), **Super+H** (thu nhỏ cửa sổ): đều bị vô hiệu *(bổ sung từ bản cập nhật 2026-07-27)*.
...
> 🔧 **Cập nhật 2026-07-27 — bổ sung cấu hình chặn cử chỉ cảm ứng:** với các bản cài mới, ô tick **"Ẩn nút Activities"** cài thêm tiện ích **được cấu hình để vô hiệu** màn hình Activities overview theo mọi cách gọi — kể cả khi mở bằng **cử chỉ cảm ứng** (vuốt nhiều ngón, vuốt mép, chạm-giữ) hay di góc màn hình. Cần **khởi động lại máy** để có hiệu lực. ⚠️ Cơ chế này **cần được nghiệm thu trực tiếp tại máy** (thử tay các cử chỉ trên màn cảm ứng thật) trước khi coi là đã chặn xong — không thể kiểm chứng từ xa. Đây là vá bổ sung cho khoá dconf (chỉ chặn được phím tắt). *Về lâu dài, cách bền vững nhất vẫn là dùng phiên kiosk chuyên dụng của hệ điều hành — sẽ cân nhắc khi nâng cấp Ubuntu.*
...
- **Nếu phần mềm kiosk chưa được cài (chưa có file chạy):** dịch vụ vẫn kiên trì thử lại **mỗi 10 giây** (không bao giờ tự dừng hẳn — thiết kế cho máy không người trực); sau khi cài phần mềm, lần thử kế tiếp sẽ chạy thành công mà không cần thao tác gì thêm. Nhịp 10 giây đủ chậm để không làm đầy nhật ký hệ thống. Xem trạng thái: `systemctl --user status ipgs-kiosk-app.service`.
