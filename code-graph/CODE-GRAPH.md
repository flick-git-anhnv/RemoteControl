...
| SSH installers | `CcuClient/*Service.cs` | Cài `.deb`/`.sh`, deploy ZcuAgent (systemd), kiosk setup — mọi tham số qua `ShellQuote`. `KioskDeployOptions.LockdownShell` (F09, tham số 11 của `2-configure-system.sh`): khoá lối thoát kiosk bằng dconf system-db + lock (`/etc/dconf/db/local.d/00-kiosk-lockdown` + `locks/` + `/etc/dconf/profile/user`) — cần reboot để có hiệu lực (G020) | `RemoteAppInstallService`, `ZcuRemoteInstallerService`, `KioskDeployService` |
