...
| SSH installers | `CcuClient/*Service.cs` | Cài `.deb`/`.sh`, deploy ZcuAgent (systemd), kiosk setup — mọi tham số qua `ShellQuote`. `KioskDeployOptions.LockdownShell` (F09, tham số 11 của `2-configure-system.sh`): khoá lối thoát kiosk bằng dconf system-db + lock (`/etc/dconf/db/local.d/00-kiosk-lockdown` + `locks/` + `/etc/dconf/profile/user`) — cần reboot để có hiệu lực (G020). `KioskDeployOptions.EnableWatchdog` (F10, tham số 12): systemd USER service `ipgs-kiosk-app.service` (`Restart=always` `RestartSec=10` `StartLimitIntervalSec=0` — R2: không bao giờ vào failed vĩnh viễn, kiosk không người trực) tự khởi động lại app kiosk; khi bật thì XÓA autostart `.desktop` của app bất kể tham số autostart (R1 — ma trận trạng thái nhất quán). Cử chỉ cảm ứng mở overview (F10) chặn bằng extension GNOME Shell cục bộ `disable-overview-gestures@kztek` nhúng trong `1-install-software.sh` (theo ô `HideActivities`) | `RemoteAppInstallService`, `ZcuRemoteInstallerService`, `KioskDeployService` |
