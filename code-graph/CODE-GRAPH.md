...
| SSH installers | `CcuClient/*Service.cs` | Cài `.deb`/`.sh`, deploy ZcuAgent (systemd), kiosk setup — mọi tham số qua `ShellQuote`. `KioskDeployOptions.LockdownShell` (F09, tham số 11 của `2-configure-system.sh`): khoá lối thoát kiosk bằng dconf system-db + lock (`/etc/dconf/db/local.d/00-kiosk-lockdown` + `locks/` + `/etc/dconf/profile/user`) — cần reboot để có hiệu lực (G020). `KioskDeployOptions.EnableWatchdog` (F10, tham số 12): systemd USER service `ipgs-kiosk-app.service` (`Restart=always` + `StartLimit` chống-loop) tự khởi động lại app kiosk; khi bật thì bỏ autostart `.desktop` của app. Cử chỉ cảm ứng mở overview (F10) chặn bằng extension GNOME Shell cục bộ `disable-overview-gestures@kztek` nhúng trong `1-install-software.sh` (theo ô `HideActivities`) | `RemoteAppInstallService`, `ZcuRemoteInstallerService`, `KioskDeployService` |
...
| 2026-07-27 | `scripts/linux-kiosk/1-install-software.sh` + `2-configure-system.sh` + `CcuClient/KioskDeployService.cs` + `CcuUI/Views/KioskDeployWindow.axaml(.cs)` | Fix | F10 (kiosk cảm ứng, không bàn phím): (A) extension GNOME Shell cục bộ `disable-overview-gestures@kztek` chặn cử chỉ cảm ứng mở overview (theo ô HideActivities); (B) `EnableWatchdog` — systemd USER service `ipgs-kiosk-app.service` (Restart=always + StartLimit chống-loop) + checkbox mới ở Tab Config phần mềm (tham số 12). Nhóm lỗ phím tắt ⏭️ bỏ theo quyết định user (máy không bàn phím) | senior-developer |
